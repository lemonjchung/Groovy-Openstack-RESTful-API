...
<body>
  <script defer type="text/javascript" src="${resource(dir: 'js', file: 'customBackground.js')}?v=${build}"></script>
<div class="bodyWrapper">
<div class="body cluster">
    <div class="intro">
        <g:javascript>
            function uncheck() {
                var chkBox = document.getElementById("trafficAllowed");
                chkBox.value = 0;
            }
        </g:javascript>
      <h1 color="#ff658e">Manage Cluster of Sequential Auto Scaling Groups</h1>
      <g:if test="${flash.message1}">
        ...
        <g:if test="${!runningTasks && !requireLoginForEdit}">
            <div class="buttons">
                   ...
                   <!-- <g:buttonSubmit class="deploy" value="Status/History" onclick="calltest()"  /> -->
            </div>
        </g:if>
        <br/>
    </div>
    %{--TODO: Sachin - Check if zone availability information needs to be displayed--}%
    %{-- <g:if test="${instanceType && zoneAvailabilities}">
       <table class="reservations" title="Reserved instances are cheaper and more likely to be available than on demand instances">
         ...
         <g:each var="za" in="${zoneAvailabilities}">
           <tr>
             <td><g:availabilityZone value="${za.name}"/></td>
    ...
    <ul class="groupReplacingPush">
      <g:each var="autoScalingGroup"  in="${cluster}">
        <li class="clusterAsgForm ${autoScalingGroup.seemsDisabled() ? 'disabledGroup' : ''}" style="border-style:solid;border-width:6px;border-color:${autoScalingGroup.GetASGColor()}" >
          <g:form method="post" class="validate">
            <div class="title" style="background-color:${autoScalingGroup.GetASGColor()}">
               <g:hiddenField name="name" value="${autoScalingGroup.autoScalingGroupName}"/>
               <g:hiddenField name="resizeClusterURL" value="${createLink(controller:'cluster',action:'checkResizeStatus')}"/>
               <h2><g:linkObject type="autoScaling" name="${autoScalingGroup.autoScalingGroupName}" style="font-weight: bold;color:white"/></h2>
            </div>
            <div class="expiration">${autoScalingGroup.status}</div>
            ...
            <g:else>
              <div class="buttons">
                <g:if test="${loadBalancers == null || loadBalancers.isEmpty() || autoScalingGroup.status.indexOf("FAILED") > 0}">
                    <g:buttonSubmit class="delete" action="delete" value="Delete" disabled="false"
                  ...
                  <g:buttonSubmit class="requireLogin trafficEnable" action="activate" value="Enable" disabled="true"/>
                  <g:buttonSubmit class="requireLogin trafficDisable" action="deactivate" value="Disable"
                                  disabled="true"/>
                </g:if>
                <g:elseif test="${autoScalingGroup.seemsDisabled()}">
                    <g:buttonSubmit class="delete" action="delete" value="Delete" disabled="false"
                                    data-warning="Terminate: ${autoScalingGroup.instances.size()} instances and delete Auto Scaling Group '${autoScalingGroup.autoScalingGroupName}'?"/>
                  <g:buttonSubmit class="requireLogin trafficEnable" action="activate" value="Enable" disabled="false"/>
                  <g:buttonSubmit class="requireLogin trafficDisable" action="deactivate" value="Disable" disabled="true"/>

                </g:elseif>
                <g:else>
                    <g:buttonSubmit class="delete" action="delete" value="Delete" disabled="true"
                                    data-warning="Terminate: ${autoScalingGroup.instances.size()} instances and delete Auto Scaling Group '${autoScalingGroup.autoScalingGroupName}'?"/>
                  <g:buttonSubmit class="requireLogin trafficEnable" action="activate" value="Enable" disabled="true"/>
                  <g:buttonSubmit class="requireLogin trafficDisable" action="deactivate" value="Disable" disabled="false"/>
                </g:else>
              </div>
                <div class="buttons requireLogin">
                  %{-- onclick="resizeCluster(${autoScalingGroup.autoScalingGroupName}, ${group?.template?.getASG()?.properties?.maxSize}); return false;"--}%
                <g:if test="${loadBalancers == null || loadBalancers.isEmpty()|| autoScalingGroup.status.indexOf("FAILED") > 0}">
                    <g:buttonSubmit class="resize" action="resize" value="Resize" asgName="${autoScalingGroup.autoScalingGroupName}" disabled="true"/>
                </g:if>
                <g:else>
                    <g:buttonSubmit class="resize" action="resize" value="Resize" asgName="${autoScalingGroup.autoScalingGroupName}" disabled="${autoScalingGroup.seemsDisabled() ? 'true' : 'false'}"/>
                </g:else>
                    <label for="minAndMaxSize_${autoScalingGroup.autoScalingGroupName}">to</label>
                    <g:if test="${autoScalingGroup.minSize == autoScalingGroup.maxSize}"  >
                        <input type="number" size="2" class="groupSize number"
                               id="minAndMaxSize_${autoScalingGroup.autoScalingGroupName}" name="minAndMaxSize"
                               value="${minSize == null ? 1 : minSize}"  min="1"
                               onchange="modifyMax()" onblur="validateNumber(event)" max="999"/> per Availability Zone
                    </g:if>
                    <g:else>
                        <input type="number" size="2" class="groupSize number"
                               id="minSize_${autoScalingGroup.autoScalingGroupName}" name="minSize"
                               value="${minSize == null ? 1 : minSize}"  min="1"
                               onchange="modifyMax()" onblur="validateNumber(event)" max="999"/>
                        <label for="minSize_${autoScalingGroup.autoScalingGroupName}">min</label> /
                        <input type="number" size="2" class="groupSize number"
                               id="maxSize_${autoScalingGroup.autoScalingGroupName}" name="maxSize"
                               value="${maxSize == null ? 1 : maxSize}"  min="1"
                               onchange="modifyMax()" onblur="validateNumber(event)" max="999"/>
            ...
            <table class="tiny">
              ...
              <caption style="color:black">${total} instance${total == 1 ? '' : 's grouped by state'}</caption>
              <tr>
                <th>Count</th>
                <th>State</th>
                <th>Build</th>
              ...
              <g:each in="${instanceStates}" var="instanceState">
                ...
                <tr>
                  ...
                  <td>
                    <g:if test="${instanceState.buildNumber}">
                        <g:linkObject type="image"  style="color:black" name= "${instanceState.buildNumber}" id="${instanceState.imageId}"/>
                    </g:if>
                    <g:else>
                      <g:linkObject type="image" style="color:black" name= "${instanceState.imageId}" id="${instanceState.imageId}"/>
                    </g:else>
                  </td>
                    <td>
                        <g:each in="${loadBalancers}" var="elbName" status="i">
                            <g:linkObject type="loadBalancer" name="${elbName.name}" id="${elbName.id}" compact="true"/>
                ...
                <tr class="advancedItems">
                  <td colspan="100%" style="color:black">
                    <g:each in="${statesToInstanceLists[instanceState]}" var="instance">
                      <g:linkObject type="instance" name="${instance.instanceId}" style="color:black"/>
                    </g:each>
                  </td>
                </tr>
                </tbody>
          ...
          <g:if test="${!requireLoginForEdit}">
            <g:form method="post" class="validate" action="createNextGroup">
              ...
              <table>
                ...
                <g:render template="/loadBalancer/selection" model="[isRequired = true]"/>
